...
<div class="page page-center">
      <div class="container-tight py-2">
        <div class="text-center mb-4">
            <h1>Modalit&eacute; d'accueil des publics</h1>
        </div>
        <p class="text-center mb-3">Sélectionnez les moyens par lesquels vous recevez vos publics :</p>
        <form class="card card-md" id="myForm" action="<?= base_url("/inscription/structure/inscriptionStructureModalitePublicSave"); ?>" method="POST">
        <?php if (session()->has("error_nb_modalite")) { ?>
            <div class="alert alert-danger d-flex align-items-center" role="alert">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-triangle-fill" viewBox="0 0 16 16">
                    <path d="M8.982 1.566a1.13 1.13 0 0 0-1.96 0L.165 13.233c-.457.778.091 1.767.98 1.767h13.713c.889 0 1.438-.99.98-1.767L8.982 1.566zM8 5c.535 0 .954.462.9.995l-.35 3.507a.552.552 0 0 1-1.1 0L7.1 5.995A.905.905 0 0 1 8 5zm.002 6a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
                </svg> &nbsp;&nbsp;
                <div>
                    <?= session("error_nb_modalite") ?>
                </div>
            </div>
        <?php } ?>
        ...
        <div class="pb-3 pt-2">
            <div class="form-selectgroup form-selectgroup-pills">
                <?php foreach($cs_modalites as $cs_modalite){?>
                    <label class="form-selectgroup-item">
                        <input type="checkbox" id="<?= $cs_modalite['id'] ?>" name="id_modalite[]" value="<?= $cs_modalite['id'] ?>" class="form-selectgroup-input">
            ...
            <div class="row align-items-center mt-3">
                    <div class="col-auto">
                            <a href="<?= base_url("/inscription/structure/inscriptionStructureZone"); ?>" class="text-secondary text-decoration-none">
...
function handleCheckboxChange() {
  ...
  // Get the saved checkbox values from local storage
        var savedCheckboxes = localStorage.getItem("modalite");
        ...
        localStorage.setItem("modalite", JSON.stringify(checkedCheckboxes));
        });
    </script>