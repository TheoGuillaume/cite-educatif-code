...
-- Database: `temp_data`
--
-- CREATE DATABASE IF NOT EXISTS `temp_data` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
--USE `temp_data`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`temp_data`@`%` SQL SECURITY DEFINER VIEW `passage_quotidien`  AS SELECT coalesce(sum(`t1`.`nombre_de_passages`),0) AS `somme_passages`, `t2`.`email` AS `email` FROM (`c_utilisateur` `t2` left join `c_utilisateur` `t1` on((`t1`.`id` = `t2`.`id`))) GROUP BY `t2`.`id` ORDER BY `somme_passages` DESC LIMIT 0, 5 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`temp_data`@`%` SQL SECURITY DEFINER VIEW `structure_categorie`  AS SELECT count(`str`.`id`) AS `nbr`, `cat`.`nom` AS `nom` FROM (`cs_struct` `str` join `cs_categorie` `cat` on((`cat`.`id` = `str`.`id_categorie`))) GROUP BY `str`.`id_categorie` ;
...
-- 20230230 Mbolatiana cs_champ_action ok
...
('Accompagnement médicale et thérapeutique', 1)
;
